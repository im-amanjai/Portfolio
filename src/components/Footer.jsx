    ...
    >
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-10">
        ...
        <div className="text-center md:text-left order-2 md:order-1">
          <h1 style={{ color: theme.textMain }} className="text-2xl font-black tracking-tighter italic uppercase">
            Aman <span style={{ color: theme.primary }}>Jaiswal.</span>
          </h1>
          <p style={{ color: theme.textSecondary }} className="mt-2 text-sm max-w-xs font-medium opacity-80">
            Full-Stack Developer building scalable MERN applications and backend systems.
          </p>
        </div>
        ...
        <div className="flex items-center gap-4 order-3">
          <IconLink href="https://linkedin.com/in/aman-jaiswal-504ab3258" label="LinkedIn" icon={<FiLinkedin />} theme={theme} />
          <IconLink href="https://github.com/im-amanjai" label="GitHub" icon={<FiGithub />} theme={theme} />
          <IconLink href="https://wa.me/+919569895155" label="WhatsApp" icon={<FiMessageCircle />} theme={theme} />
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto mt-12 pt-8 border-t border-dashed" style={{ borderColor: theme.border }}>
        <div style={{ color: theme.textSecondary }} className="text-center text-[10px] md:text-xs font-black uppercase tracking-[0.3em] opacity-50">
          © {new Date().getFullYear()} Aman Jaiswal • All Rights Reserved
        </div>
      </div>
    </footer>
  );
